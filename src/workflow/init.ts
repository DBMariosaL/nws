...
import {
  loadWorkspaceRoot,
  saveWorkspaceRoot,
} from "../state/workspace-root.js";
import { maskToken } from "../utils/mask.js";
import {
  promptForDataSource,
  promptForRootInput,
  promptForToken,
} from "../cli/prompts/access.js";
...
function buildAccessErrorMessage(details: string): string {
  const trimmed = details.trim();
  return `${trimmed} Share the root with the integration and ensure it has read, insert, and update capabilities.`;
}

export async function initWorkflow(options: WorkflowOptions): Promise<WorkflowResult> {
  ...
  if (!interactive && !yes) {
    return {
      ...
      status: "error",
      message:
        "Non-interactive mode detected. Re-run with --yes to reuse saved settings.",
    };
  }
  ...
  if (!interactive && yes && (!existingAuth?.token || !existingRoot)) {
    return {
      ...
      status: "error",
      message:
        "Non-interactive mode requires saved token and root. Run init interactively first.",
    };
  }
  ...
  const rootForVerification =
    resolvedRoot.kind === "database"
      ? {
          ...resolvedRoot,
  ...
  const capabilityVerification = await verifyRootCapabilities(
    client,
    rootForVerification
  );
  if (!capabilityVerification.ok) {
    return {
      command: "init",
      status: "error",
      message: buildAccessErrorMessage(
        `Access verification failed. ${capabilityVerification.summary}`
      ),
    };
  }
  ...
  if (resolvedRoot.kind === "page") {
    ...
    await saveWorkspaceRoot({
      ...
      data_source_id:
        selectedDataSourceId ?? resolvedRoot.data_source_ids[0],
      title: resolvedRoot.title,
      url: resolvedRoot.url,
    });
  }
